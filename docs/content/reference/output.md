...
describes what users get. Pick a format with `-o`, or let cocktaildb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cocktaildb <command> -o table   # aligned columns for reading
cocktaildb <command> -o jsonl   # one JSON object per line, for piping
cocktaildb <command> -o json    # a single JSON array
cocktaildb <command> -o csv     # spreadsheet friendly
cocktaildb <command> -o tsv     # tab-separated
cocktaildb <command> -o url     # just the URL column
cocktaildb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cocktaildb <command> --fields id,title,url
```
...
```bash
cocktaildb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cocktaildb <command>            # a table, because this is a terminal
cocktaildb <command> | wc -l    # JSONL, because this is a pipe
```
