// Package cli assembles the cocktaildb command tree from the cocktaildb
...
// NewApp assembles the kit application from the cocktaildb domain. The
...
//
// To add a command, declare it in cocktaildb/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := cocktaildb.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(cocktaildb.Domain{}).Register(app)
